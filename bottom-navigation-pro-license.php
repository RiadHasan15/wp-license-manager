<?php
/**
 * Complete License Integration for bottom-navigation-pro
 * Version: 2.0.0
 */
...
class BOTTOM_NAVIGATION_PRO_License_Manager {
    ...
    private $license_key;
    private $product_slug = 'bottom-navigation-pro';
    ...
    public function __construct($plugin_file, $version) {
        ...
        $this->license_key = get_option($this->product_slug . '_license_key');
    ...
    public function license_page() {
        ...
        $status = get_option($this->product_slug . '_license_status');
        $last_check = get_option($this->product_slug . '_license_last_check');
            ...
            <div class="card" style="max-width: 600px; margin-top: 20px;">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        ...
                        <td><?php echo esc_html($this->license_server_url); ?></td>
                    </tr>
                    <tr>
                        <th scope="row">Product Slug</th>
                        <td><?php echo esc_html($this->product_slug); ?></td>
                    </tr>
                </table>
            </div>
        </div>
    ...
    public function activate_license() {
        ...
        if (is_wp_error($response)) {
            update_option($this->product_slug . '_license_status', 'error');
        ...
        if (isset($body['success']) && $body['success']) {
            ...
            update_option($this->product_slug . '_license_status', 'valid');
            update_option($this->product_slug . '_license_last_check', time());
            
            add_action('admin_notices', function() {
                echo '<div class="notice notice-success"><p>License activated successfully! Updates are now enabled.</p></div>';
            });
        } else {
            update_option($this->product_slug . '_license_status', 'invalid');
    ...
    public function deactivate_license() {
        ...
        if (!is_wp_error($response)) {
            ...
            if (isset($body['success']) && $body['success']) {
                update_option($this->product_slug . '_license_status', 'inactive');
                add_action('admin_notices', function() {
            ...
            // Even if server request fails, mark as inactive locally
            update_option($this->product_slug . '_license_status', 'inactive');
            add_action('admin_notices', function() {
    ...
    public function plugins_api_filter($res, $action, $args) {
        ...
        return (object) array(
            ...
            'sections' => array(
                'description' => 'Premium navigation plugin with advanced features and customization options.',
    ...
    private function get_remote_version() {
        ...
        $cache_key = $this->product_slug . '_remote_version';
    ...
    private function get_download_url() {
        ...
        return $this->license_server_url . '/wp-json/licensing/v1/update-download?license_key=' . 
               urlencode($license_key) . '&product_slug=' . urlencode($this->product_slug);
    }
    ...
    public function license_notices() {
        ...
        if (!$current_screen || strpos($current_screen->id, 'bottom-navigation') === false) {
            return;
        }
        
        if ($status !== 'valid') {
            echo '<div class="notice notice-warning is-dismissible">';
            echo '<p><strong>Bottom Navigation Pro:</strong> ';
            
            if ($status === 'expired') {
                echo 'Your license has expired. <a href="' . admin_url('options-general.php?page=' . $this->product_slug . '-license') . '">Renew your license</a> to continue receiving updates.';
            } else {
                echo 'Please <a href="' . admin_url('options-general.php?page=' . $this->product_slug . '-license') . '">activate your license</a> to enable automatic updates.';
            }
...
// Uncomment and modify the path to match your main plugin file
